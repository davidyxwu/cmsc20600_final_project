...
class blue(object):
    ...
    def __init__(self):
        ...
        # init rospy node
        rospy.init_node("blue")

        # set up ROS and CV bridge
        self.bridge = cv_bridge.CvBridge()

        # subscribe to the robot's RGB camera data stream
        self.image_sub = rospy.Subscriber('/blue/camera/rgb/image_raw',
                        Image, self.image_callback)

        # subscribe to robot's laser scan
        rospy.Subscriber("/blue/scan", LaserScan, self.scan_callback)

        rospy.sleep(1)
        # the interface to the group of joints making up the turtlebot3
        # openmanipulator arm
        self.move_group_arm = moveit_commander.MoveGroupCommander("arm", robot_description="blue/robot_description", ns="/blue")

        # the interface to the group of joints making up the turtlebot3
        # openmanipulator gripper
        self.move_group_gripper = moveit_commander.MoveGroupCommander("gripper", robot_description="blue/robot_description", ns="/blue")
        ...
        self.cmd_vel_pub = rospy.Publisher('/blue/cmd_vel',
                        Twist, queue_size=1)

        self.twist = Twist()

        # Set up publisher for initial pose
        self.init_pose_pub = rospy.Publisher('/blue/initialpose', PoseWithCovarianceStamped, queue_size=10)
        rospy.sleep(1)
        self.init_pose()
    ...
    def init_pose(self):
        ...
        q = quaternion_from_euler(0, 0, 3.1415)
